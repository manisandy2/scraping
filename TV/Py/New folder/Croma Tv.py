...
wb = load_workbook(r"C:\Durai\TV\Urls\Croma Tv Url.xlsx")
ws = wb.active
...
for r in range(2,ws.max_row+1):
    if ws.cell(row=r,column=4).value != "N/A":
        ...
        name = driver.find_element_by_class_name("pd-title-normal")
        print(name.text)
        new_ws.cell(row=l,column=1).value = name.text

        price = driver.find_element_by_class_name("new-price")
        print(price.text)

        new_ws.cell(row=l,column=2).value = price.text
        new_wb.save(r"C:\Durai\TV\Save Data\Croma Tv  "+date+".xlsx")
        l = l+1
